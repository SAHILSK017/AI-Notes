...
export default function NotesPage() {
  ...
  const filteredNotes = notes
    ? notes.filter(note => {
        if (activeTab === 'favorites') return note.isFavorite === true;
        if (activeTab === 'shared') return note.isPublic === true;
        return true;
      })
    : [];
  ...
  const openDeleteModal = (noteId) => setDeleteModal({ isOpen: true, noteId });
  const closeDeleteModal = () => setDeleteModal({ isOpen: false, noteId: null });
    ...
    <div className="p-6 md:p-12 pb-24 animate-in fade-in duration-1000">
      ...
      <div className="flex items-center justify-between mb-10 px-4 py-2 border-b border-border/40">
        <div className="flex items-center gap-8">
          {[
            { id: 'all', label: `All Entries (${notes?.length || 0})` },
            { id: 'favorites', label: `Favorites (${notes?.filter(n => n.isFavorite).length || 0})` },
            { id: 'shared', label: `Shared (${notes?.filter(n => n.isPublic).length || 0})` },
          ].map(tab => (
            <button
              key={tab.id}
...
function NoteCard({ note, onFavorite, onEdit, onDelete }) {
  return (
    <div className="group relative h-full">
      <div className="absolute top-6 right-6 flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-all duration-300 z-20 translate-y-2 group-hover:translate-y-0">
        <button
          onClick={(e) => { e.stopPropagation(); e.preventDefault(); onFavorite(note._id, note.isFavorite); }}
        ...
        <button
          onClick={(e) => { e.stopPropagation(); e.preventDefault(); onEdit(note._id); }}
        ...
        <button
          onClick={(e) => { e.stopPropagation(); e.preventDefault(); onDelete(note._id); }}
          ...
          <div className="flex flex-col h-full relative z-10">
            <div className="flex items-start justify-between mb-8">
              ...
              <div className="flex items-center gap-2">
                {note.isFavorite && <Heart className="w-4 h-4 text-rose-500 fill-current animate-in zoom-in-75" />}
                {note.aiSummary && (
                  <div className="flex items-center gap-2">
...
function NoteListItem({ note, onFavorite, onEdit, onDelete }) {
  return (
      ...
      <Link href={`/notes/${note._id}`} className="block">
        <div className="p-6 bg-card/65 backdrop-blur-sm border border-border rounded-3xl hover:bg-card hover:border-primary/30 transition-all duration-500 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="flex items-center gap-5 min-w-0 flex-1">
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-3 mb-1.5 flex-wrap">
                ...
                {note.isFavorite && <Heart className="w-3.5 h-3.5 text-rose-500 fill-current shrink-0 animate-in zoom-in-75" />}
                {note.aiSummary && <Sparkles className="w-3.5 h-3.5 text-primary shrink-0" />}
              </div>
          ...
          <div className="flex items-center gap-6 shrink-0 justify-between md:justify-end">
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={(e) => { e.preventDefault(); e.stopPropagation(); onFavorite(note._id, note.isFavorite); }}
              ...
              <button
                onClick={(e) => { e.preventDefault(); e.stopPropagation(); onEdit(note._id); }}
