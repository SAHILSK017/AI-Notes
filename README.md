...
### 💡 Interactive Vintage Desk Lamp & Cinematic Auth Switch
- **Physical Switch Chain:** Click either the glass shade or the pull chain to toggle the workspace ON/OFF.
- **GSAP Elastic Animations:** The pull chain features realistic vertical translation and elastic bounce-back recoil animations.
- **Web Audio Synthesis:** Sound feedback is dynamically generated using the native browser Web Audio API to create a realistic physical switch click, requiring no local audio assets or network requests.
- **Cinematic Box Toggle:** The authentication card (Login / Signup) is completely hidden when the lamp is OFF (`opacity-0 translate-y-4 scale-95 pointer-events-none`) and transitions into focus with a golden border and warm ambient glow when switched ON.
- **Responsive Layout Placement:** On desktop screens, the lamp overlays the wooden table in the left cinematic panel (`bottom-[26%] right-[7%]`). On mobile viewports, the left panel is hidden and the lamp stacks above the form card in the right panel to maintain full accessibility.
- **Warm Amber Ambient Shifts:** Switching the lamp ON changes the right panel background to a warm chocolate/dark amber shade (`#13100b`) and casts a golden radial aura.

### ✅ Persistent AI-Generated Tasks
- **Database-Level Persistence:** Task lists generated by the AI are permanently saved directly inside the MongoDB `Note` documents in the `aiActionItems` sub-document array.
- **Instant Completion Sync:** Checking/unchecking a task immediately fires an API patch to persist the state in the database, ensuring completion states survive page refreshes, routes, and workspace switching.
- **Smart Merging Engine:** Regenerating AI insights or editing the note contents merges new items with existing tasks, keeping completed/uncompleted states intact without producing duplicate entries.
